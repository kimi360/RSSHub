...
const parseItems = ($, ctx) =>
    $('.movie-box')
        ...
        ${detail.videoplayer ? `<h3>视频预览 : </h3><br />${detail.videoplayer}` : ''}
    `.trim(),
            };
        });
...
const getDetail = async (link, ctx) => {
    ...
    if (cache) {
        ...
        if (videos.length > 0) {
            ...
            videoplayer = `<video controls="controls" width="800" height="538" poster="${preview_url}" src="${embedded_url}"> </video>`;
        } else {
            videoplayer = null;
        }

        const detail = { actresses, filmId, filmTime, filmLast, filmEstabName, filmMakerbName, seriesName, directorName, code, screenData, magnetlinks, videoplayer };
