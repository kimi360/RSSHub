...
const parseItems = ($, ctx) =>
    $('.movie-box')
        ...
        ${detail.magnetlinks ? `<h3>下载地址 : </h3>${detail.magnetlinks}` : ''}
    `.trim(),
            };
        });
    ...
    if (cache) {
        ...
        if (match) {
            magnetlinks = match.map(function(str){
                ...
                return "<br \/><a href=\""+mlink+"\">"+mname+" | "+msize+" | "+ mlink +"<\/a>";
            }
            );
...
/*         let magnetlink = [];
        let magnetname = [];
        if (match) {
            magnetlink = match.map((i) => /td width="70%" onclick="window.open\('(magnet.*?)',/mg.exec(i)[1]);
