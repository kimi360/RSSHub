...
const parseItems = ($, ctx) =>
    $('.movie-box')
        ...
        ${detail.magnetlinks ? `<h3>下载地址 : </h3><hr \/>${detail.magnetlinks.join('')}` : ''}
    `.trim(),
            };
        });
    ...
    if (cache) {
        ...
        if (match) {
            magnetlinks = match.map(function(str){
                ...
                return "<div style=\"word-wrap:break-word\"><a href=\""+mlink+"\">"+mname+" -- "+msize+ "<\/a><\/div><div style=\"word-wrap:break-word\">"+ mlink + "<\/div><hr \/>";
            }
            );
        } else {
            magnetlinks = null;
