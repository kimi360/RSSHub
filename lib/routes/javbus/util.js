...
const parseItems = ($, ctx) =>
    $('.movie-box')
        ...
        ${detail.magnetlinks ? `<h3>下载地址 : </h3><table border="1" cellspacing="0" cellpadding="0">${detail.magnetlinks.join('')}</table>` : ''}
    `.trim(),
            };
        });
...
const getDetail = async (link, ctx) => {
    ...
    if (cache) {
        ...
        let filmGid = /gid = ([0-9]*?);/.exec(detailPage);
        let filmUc = /uc = ([0-9]*?);/.exec(detailPage);
        let filmImg = /var img = '(.*?)';/.exec(detailPage);
        let filmFloor = Math.floor(Math.random()*1000+1)
        ...
        match = magnettable.match(/<tr onmouseover=[\s\S]*?<\/tr>/mg);
        let magnetlinks = [];
        if (match) {
            magnetlinks = match.map(function(str){
                //magnet地址
                let mlink = /td width="70%" onclick="window.open\('(magnet.*?)',/.exec(str)[1];
                //console.log(str);
                //提取a链接
                let mrow = str.match(/<a [\s\S]*?magnet[\s\S]*?">([\s\S]*?)</mg);
                //magnet名称
                let mname = /<a [\s\S]*?magnet[\s\S]*?">([\s\S]*?)</.exec(mrow[0])[1].trim();
                //magnet大小
                let msize = /<a [\s\S]*?magnet[\s\S]*?">([\s\S]*?)</.exec(mrow[1])[1].trim();
                return "<tr><td><a href=\""+mlink+"\">"+mname+"<\/td><td>"+msize+ "<\/a><\/td><\/tr><tr><td style=\"word-wrap:break-word\" colspan=\"2\">"+ mlink + "<\/td><\/tr>";
            }
            );
        } else {
            magnetlinks = null;
        ...
        const detail = { actresses, filmTime, filmLast, filmEstabName, filmMakerbName, seriesName, directorName, code, screenData , magnetlinks };
